...
export const translate = async ({
  ...
  to =
    to ||
    (from === preferredLanguage ? secondPreferredLanguage : preferredLanguage);

  if (!(from in langCode) || !(to in langCode)) {
  ...
  try {
    ...
    if (
      ...
      !originTo &&
      data.src === to &&
      preferredLanguage !== secondPreferredLanguage
    ) {
      params.sl = data.src;
    ...
    const result = {
      ...
      result: data.sentences
        ?.reduce((t, c) => `${t}${c.trans ?? ''}`, '')
        .split('\n')
        .filter((v) => v.trim()),
      dict: data.dict?.reduce(
        (t, v) => t.concat(v.pos + ': ' + v.terms.join(', ')),
        []
      ),
      phonetic:
        data.sentences?.[1]?.src_translit &&
        `[${data.sentences[1].src_translit}]`,
      related: data.related_words?.word,
      example: data.examples?.example?.slice(0, 3).map((v) => v.text),
    };
...
const fetchGoogle = async (params) => {
  ...
  let res = await fetch(url + getQueryString(params)).catch(() => {
  ...
  if (!res.ok) {
    if (res.status === 429 && expiry < timpstamp) {
      ...
      res = await fetch(url + getQueryString(params)).catch(() => {
        throw getError(CONNECTION_TIMED_OUT);
      });
